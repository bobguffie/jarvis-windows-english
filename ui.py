    ...
    def start_ambient(self):
        ...
        with self._lock:
            if not self._enabled:
                return
            if self._foreground_proc and self._foreground_proc.is_playing():
                return
...
class JarvisUI:
    ...
    def switch_workspace_tab(self, tab: str) -> str:
        """Switch the workspace card between 'todo', 'media', and 'smarthome' views.

        Args:
            tab: "todo", "media", "smarthome", or "home assistant"

        Returns:
            A confirmation message for the AI.
        """
        norm = (tab or "").strip().lower()
        # Accept "home assistant" as an alias for "smarthome"
        if norm in ["home assistant"]:
            norm = "smarthome"
        allowed_tabs = ("todo", "media", "smarthome")
        if norm not in allowed_tabs:
            return f"Unknown workspace tab: {tab}. Use 'todo', 'media', or 'smarthome'."
        ...
        LABEL_MAP = {"todo": "CHECKLIST", "media": "MEDIA", "smarthome": "SMARTHOME"}
    ...
    def _update_stats(self):
        try:
            ...
            self._stats['disk'] = psutil.disk_usage(drive).percent
    ...
    def _draw_left_panel(self, c):
        ...
        if self._active_workspace_tab == "todo":
            ws_title = "WORKSPACE: CHECKLIST"
        elif self._active_workspace_tab == "media":
            ws_title = "WORKSPACE: MEDIA"
        elif self._active_workspace_tab == "smarthome":
            ws_title = "WORKSPACE: SMARTHOME"
        else:
            ws_title = "WORKSPACE: CHECKLIST"
        cards = [
        ...
        for (section, _, title, accent), ph in zip(cards, heights):
            ...
            if section == "time":
                ...
                hy = current_y + 48
                for line in self._workspace_lines[:5]:
    ...
    def _draw(self):
        ...
        c.create_text(W//2, H-13, fill=C_DIM, font=font_body(9),
                      text="JARVIS · Windows v2.1 Edition · Realtime Voice Core")
